...
BLEFloatCharacteristic signalCharacteristicX(BLE_UUID_SIGNAL_X, BLERead | BLENotify);
...
<<<<<<< HEAD
    ...
    signalCharacteristicX.writeValue(0.0);
    ...
    if (central)
    {
        ...
        while (central.connected())
        {
            if (IMU.accelerationAvailable() && IMU.gyroscopeAvailable())
            {
                ...
                {
                    // Serial.println("Current State: Standing");
                    signalCharacteristicX.writeValue(1.0);
                }
                else
                {
                    // Serial.println("Current State: Not Standing");
